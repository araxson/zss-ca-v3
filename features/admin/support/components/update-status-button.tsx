...
function getStatusData(status: string): { value: string; label: string; variant: 'default' | 'secondary' | 'outline' } {
...
export function UpdateStatusButton({
  ticketId,
  currentStatus,
}: UpdateStatusButtonProps): React.JSX.Element {
  ...
  function handleStatusSelect(newStatus: string): void {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                disabled={loading}
                className="w-full gap-2 justify-between sm:w-auto sm:min-w-[14rem]"
