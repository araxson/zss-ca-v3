...
export async function bulkCreateNotificationAction(
  data: unknown
): Promise<{ error: string; fieldErrors?: Record<string, string[]> } | { error: null; data: { count: number } }> {
  // 1. Validate input with Zod
  const result = bulkCreateNotificationSchema.safeParse(data)
  ...
  return { error: null, data: { count: clients.length } }
}
