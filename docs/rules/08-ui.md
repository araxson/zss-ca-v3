...
### Pattern 1: Component Selection (Semantic Over Generic)
**When to use:** Every time you need a UI element
**Implementation:**
```tsx
// ❌ WRONG - Repetitive Card usage
<Card>
  <CardHeader>
...
<Item variant="outline">
  <ItemContent>
    ...
    <ItemDescription>$1000 this month</ItemDescription>
  </ItemContent>
</Item>
...
<Empty>
  ...
  <EmptyDescription>Start adding entries to see stats.</EmptyDescription>
</Empty>
```

**Decision Guide:**
- **Stats/Metrics** → Chart components (not Card)
- **Navigation** → Tabs/Accordion/Navigation Menu
- **User Actions** → Alert/Dialog/Sheet
- **Lists/Data** → Table/DataTable or Item + ItemGroup
- **Empty States** → Empty component (not Card)
- **Forms** → Field component (not Card)

---

### Pattern 2: Slot Styling (Leave Structural Slots Pristine)
**When to use:** Always - default behavior for all shadcn components
**Implementation:**
```tsx
// ❌ WRONG - Custom styling on structural slots
<Card>
  <CardTitle className="text-2xl font-bold">Title</CardTitle>
  <CardDescription className="text-gray-500">Description</CardDescription>
  <DialogTitle className="text-lg">Dialog</DialogTitle>
  <AlertDialogDescription className="text-sm">Alert text</AlertDialogDescription>
</Card>

// ✅ CORRECT - No className on structural slots
<Card>
...
<Dialog>
  <DialogContent>
    <DialogHeader>
      ...
      <DialogDescription>Alert text</DialogDescription>
    </DialogHeader>
  </DialogContent>
</Dialog>
```
...
### Pattern 12: Theme Customization (CSS Variables)
**When to use:** Brand colors, custom themes, multi-tenant applications
**Implementation:**
```css
/* app/globals.css */
@import "tailwindcss";
...
@theme {
  ...
  --color-brand-900: oklch(0.53 0.12 118.34);
}

/* Override default theme colors */
:root {
  --radius: 0.5rem; /* Custom border radius */
  --primary: oklch(0.208 0.042 265.755); /* Brand primary */
...
```tsx
// Usage in components (automatic via CSS variables)
<Button>Primary Action</Button> {/* Uses --primary color */}
...
### Pattern 15: Dynamic CSS Variables with Tailwind (Tailwind v4)
**When to use:** Dynamic theming, component-level styling
**Implementation:**
```tsx
// ✅ CORRECT - Inline CSS variables with Tailwind utilities
export function BrandedButton({
  ...
  return (
    <button
      ...
      className="bg-(--bg-color) text-(--text-color) hover:bg-(--bg-color-hover) rounded-md px-4 py-2"
    >
      {children}
    </button>
  )
...
// ✅ CORRECT - Arbitrary CSS variable utilities
<div className="[--gutter-width:1rem] lg:[--gutter-width:2rem]">
  <div className="px-[--gutter-width]">
    {/* Content with responsive gutter */}
  </div>
</div>
```

**When allowed:**
- Setting CSS variables via `style` prop: ✅ ALLOWED
- Direct style object for other properties: ❌ FORBIDDEN (use Tailwind utilities)
